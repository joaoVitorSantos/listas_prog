...
def media_final_aprovado_reprovado(p1, p2, ep1, ep2):
    ...
    media_exercicios = ep1 + ep2 / 2
    media_final = media_provas * 0.7 + media_exercicios * 0.3
    if media_final>=7:
        return True
    else:
        return False
...
def tinta(metros_pintar):
    ...
    qtd_litros = metros_pintar/3
    qtd_lata = qtd_litros / 18
    return ceil(qtd_lata)
...
def acrescimo_nota_bb(nota_sozinho, nota_com_ajuda):
    ...
    acrescimo_final = acrescimo * 0.25
    if acrescimo_final < 0:
        return 0
    else:
        return round(acrescimo_final, 1)
