...
def imprime_mes_por_extenso(data):
    ...
    return data[0:2] + ' de ' + mes_extenso + ' de ' + data[6:10]
...
def numeros_sortudos(limite_inferior=1, limite_superior=100000):
    ...
    extremos = [limite_inferior, limite_superior]
    sortudos = 0
    inicio = extremos[0]

    while inicio <= extremos[1]:
...
def é_número_válido(numero):
    ...
    if é_azarado(numero) == False and soma_é_par(numero) == True and é_chato(numero) == False:
        return True
    else:
        return False
...
def ponteironuloville(telefones):
    ...
    i = 0
    for n in telefones:
        if not é_azarado(n) and soma_é_par(n) and not é_chato(n):
            i += 1
    return i
